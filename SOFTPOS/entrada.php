...
$(document).on('click','.labelrem', function(){
...
$(document).ready(function(){
	...
	var nombre=$("#nombreprod1").text();
	var codigo=$("#imgtemp").attr("value");
	var total=canti;
	alert(total);
	$("#listaprod1").append("<li class='lifact1' id='lifact'><div class='formgroup'><label class='labelimg'><img class='imagenfact' src="+img+"></label><label id='nombre' class='labelnombre'>"+nombre+"</label><label id='micod'>"+codigo+"</label><label id='cantidad' class='labelcant'>"+canti+"</label><label class='labelrem'>X</label></div></li>'");	
	$("#centro").hide();
	$("#seleccion").show();
	$("#imgtemp").remove();
	$("#inpcant").val("");
	...
	var grantotal=parseInt(grantotal)+parseInt(total);
...
<div id="centro">
	...
	<div id="prodsen"><div class="product-header">
		<h3 class="product-title"><a href="#" id="nombreprod1"></a></h3>
		<div class="product-price">
			
			
			<span class="amount" id="precio" ></span> </a>					
		</div>
...
while($filarespu=$sentencia->fetch_array())
				...
				$img=$filarespu[2];
				$precio=$filarespu[3];
				$cantidad=$filarespu[5];
				echo "

<div class='cajaprod1'>
<div class='product-header'>
	<h3 class='product-title'><a href='#' id='np".$cod."'>".$var."</a></h3>
	<div class='product-price'>
		
			
			<a ><span class='amount' id=p".$cod." val=".$cantidad.">".$cantidad."</span> </a>					
		</div>
		...
		<!--<div class='product-category'>
			 <a href='#' rel='tag' id='precio'>".$var."</a>		</div>-->	 
	</div><!-- .product-meta -->	
...
<div id='factura'>
	<div class='titulofact'>VISTA PREVIA DE LA ENTRADA</div>
    <div id='selects' style="padding:5px;"><label style="color:#FFF;">Entrega: </label><select name='entrega' id='entrega'>
    <option selected value='0' >Seleccione la Persona que Entrega</option>
    <?php
    include ("connect.php");
	$sent=$con->query("Select * from usuarios");
	while($resu=$sent->fetch_array())
	{
		
		echo "<option value='".$resu[0]."'>".$resu[1]." ".$resu[3]."</option>";
	}
    ...
    <label style="color:#FFF;">Recibe: </label><select name='recibe' id='recibe'>
     <option selected value='0' >Seleccione la Persona que Recibe</option>
    <?php
    include ("connect.php");
	$sent=$con->query("Select * from usuarios");
	while($resu=$sent->fetch_array())
   ...
   <ul class='listaprod1' id='listaprod10'> <li class='lifact1' ><div class='formgroup'><label class='labelimg'>IMG</label><label class='labelnombre' >NOMBRE</label><label class='labelcant' >CANT</label><label class='labelrem' >REMOVER</label></div></li></ul>	
	<ul class='listaprod1' id='listaprod1'></ul>
    <div id='botonera1'><label class='labelbotonera'>TOTAL:</label><label class='labelbotonera' id='gtotal'>0</label><input value='GENERAR ENTRADA' type='submit' id='entrar'><input value='CANCELAR' type='submit' id='cancelar'></div>
	</div>
